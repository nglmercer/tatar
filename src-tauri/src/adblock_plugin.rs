use adblock::{lists::ParseOptions, FilterSet, Engine, request::Request};
use tauri::{plugin::{Builder, TauriPlugin}, Manager, AppHandle};
...
impl AdBlockState {
    ...
    fn check_url(&self, url: &str, source_url: &str, request_type: &str) -> bool {
        ...
        if let Some(wrapper) = lock.as_mut() {
            ...
            if let Some(entry) = wrapper.cache.get_mut(&cache_key) {
                if entry.timestamp.elapsed().as_secs() < 120 { // Aumentado a 2 minutos
            ...
            if let Ok(request) = Request::new(url, source_url, normalized_type) {
                ...
                wrapper.cache.insert(cache_key, CacheEntry {
                ...
                if wrapper.cache.len() > 5000 { // Reducido para mejor rendimiento
                    let now = std::time::Instant::now();
                    
                    // Primero remover entradas expiradas
                    wrapper.cache.retain(|_, v| now.duration_since(v.timestamp).as_secs() < 120);
                    
                    // Si todavía hay muchas, remover las menos usadas
                    if wrapper.cache.len() > 3000 {
                        let mut entries: Vec<_> = wrapper.cache.iter().map(|(k, v)| (k.clone(), v.clone())).collect();
    ...
    fn get_cache_stats(&self) -> (u64, u64, usize) {
        ...
        if let Some(wrapper) = lock.as_ref() {
            (wrapper.cache_hits, wrapper.cache_misses, wrapper.cache.len())
        } else {
            (0, 0, 0)
        }
    }
...
async fn download_and_cache(url: &str, cache_path: &PathBuf) -> Result<String, Box<dyn std::error::Error>> {
    match ureq::get(url)
